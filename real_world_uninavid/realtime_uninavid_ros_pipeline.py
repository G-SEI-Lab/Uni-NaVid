#!/usr/bin/env python3
"""Pipelined real-time Uni-NaVid ROS runner for robot dog control."""
...
class UniNaVidInferenceWorker:
    ...
    def _run(self) -> None:
        while not self._stop_event.is_set():
            ...
            t0 = time.monotonic()
            rospy.loginfo(
                "[uninavid] inference start reason=%s frame_seq=%d age=%.3fs anchor=%d",
                request.reason,
                frame.seq,
                frame.age_s,
...
class UniNaVidPipelineNode:
    def __init__(self) -> None:
        ...
        rospy.loginfo(
            ...
            "pipeline=true period=%.2fs motion=%.2fs settle=%.2fs still_wait=%.2fs "
